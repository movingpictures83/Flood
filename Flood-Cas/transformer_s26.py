...
def get_data():
    dataset = pd.read_csv('data/zeda/Merged.csv', index_col=0)  # header=0, parse_dates=True, squeeze=True
    dataset.fillna(0, inplace=True)
    data = dataset.iloc[:578448, 13]  # 'WS_S26' - 13th column
...
def plot_and_loss(eval_model, data_source, epoch, scaler):
    ...
    pyplot.title("Predicted & Actual Value of WS_S26", fontsize='18')
    pyplot.xlabel('Time', fontsize='16')
    pyplot.ylabel('Water Stage', fontsize='16')
    pyplot.xticks(np.arange(0, 865, 144), date, fontsize=14)
...
epochs = 120  # The number of epochs
best_model = None
